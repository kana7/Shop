$(function(){
	...
	$("#menu li").click(function (event) {
		...
		event.stopPropagation();// comme ̤a il appelle pas le onclick des LI parents 
	});
	
	$("#menu li").hover(function(){
		
		$(this).find("ul:first").children('li').show();//affiche tout les li du ul qui sont cach̩
		$(this).addClass("menu_hover");// ajoute le hover sur le li
		if(!$(this).find("ul:first").is("visible")){// si le ul dans le li est cach̩ il fait le fadin
			if(!$(this).parent().hasClass("menu")){
				$(this).find("ul:first").css("margin-left",($(this).parent().width()-1)+"px");
			}
			$(this).find("ul:first").show();// fadeIn("fast") pour un effet plus joli 
													//Peut ̻tre chang̩ par show()
